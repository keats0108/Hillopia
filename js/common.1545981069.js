...
if (typeof(COMMON_JS) == 'undefined') {
    ...
    function getRecommendInfo($el,$img,mode) {
        ...
        var width = (typeof $el.attr('data-width') != 'undefined') ? $el.attr('data-width') : 0,
            src = $img.prop('src'),
            img_resolution = '',
            img = new Image(),
            nWidth = 0, nHeight = 0,
        ...
        if(width == 0) {
            ...
            if(selectEL == 'el-menu') {
                ...
                if(src.match(el_img.attr('src')) !== null) {
                ...
                img.onload = function() {
                    ...
                    if(mode) {
                        ...
                        $('.image-content').find('img[src$="' + this_src + '"]').attr('data-original-title', recomm_str);
                    } else {
    ...
    function setMadeWithCreatorlink() {
        ...
        if($('.dsgn-body').find('.made-with-creatorlink').length > 0) {
            console.log("$('.dsgn-body').find('.made-with-creatorlink').show()");
            $('.dsgn-body').find('.made-with-creatorlink').show();
        } else {
            var str = "\
    ...
    */
        ...
        xhttp.onreadystatechange = function() {
            if (this.readyState == 4 && this.status == 200) {
                ...
                if (/^(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$/.test(ip) && sid) {
                    $.post('/template/visitor/' + sid, { ip : ip, refer :  refer }, function(r) { console.log(r); }, 'json');
                }
            }
        };
    ...
    function errorCss(css) {
        ...
        if(pStart!=(pEnd + min + max + hover + active + focus + before + after + empty + not + checked + first + last + first_letter + first_line + first_of_type + nth + holder1 + holder2 + holder3 + media_controls + select_icon + https + http)) return true;
        return false;
    }
